...
const bg_src = ref(`url(stray:///__bg__/${chart.id})`)
...
<template>
  <div class="playfield" ref="top-div">
    <img alt="" class="playfield-bg" :src="bg_src" @error="bg_error = 1" v-if="bg_error == 0" />
    <transition name="fadeout">
      <div class="pf-animations" v-if="animation_state < 3">
        <template v-if="animation_state > 0">
          <img alt="" class="pf-ani-img" src="/song.jpg" v-if="no_img" />
          <img class="pf-ani-img" :src="img_src" alt="" @error="no_img = true" v-else />
          <div class="pf-ani-inf">
            <div class="pf-ani-name">{{ chart.song.name }}</div>
            <div class="pf-ani-comp">by {{ chart.song.composer }}</div>
            <transition name="fade">
              <div class="pf-ani-diff" v-if="animation_state < 2">
        ...
        <div class="pf-ani-inf2" v-if="animation_state > 1">
          <div>
            <div>谱师</div>
    ...
    <template v-if="show_lanes">
      ...
      <div class="pf-sprite" v-if="setting.sprite">
        <a-img alt="" :src="img_src" />
      </div>
      <svg-lane />
      <div class="pf-inf">
    ...
    <transition name="fadein" v-if="show_result">
      <div class="pf-results" style="cursor:auto;">
        <div class="pr-inf">
          <div class="pr-name">{{ chart.song.name }}</div>
          <div class="pr-comp">{{ chart.song.composer }}</div>
          <img class="pr-img" alt="" src="/song.jpg" v-if="no_img" />
          <img class="pr-img" :src="img_src" alt="" v-else />
...
.pr-score-wrapper {
  width: 100%;
  height: 10vh;
  background-image: var(--darker-bgi);
  border: 2px solid white;
  display: flex;
  flex-direction: row;
  align-items: center;
