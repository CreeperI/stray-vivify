...
{   
    ...
    let noteStat,speedStat,techStat,fillStat,multiStat,totalV2,totalV3=0;
      //基础信息声明
    let bpmList:number[][]=[];
    ...
    noteStat=Math.pow(Math.max(((((note / 1.5) - (multi / 2.5)) / (length / 20)) + basepower) - 20, 0), 1.025);
    speedStat=Math.pow(Math.max(0, ((data.averageDensity * 5) + (basepower / 2)) - (utils.clamp(multi, 0, 300) / 10) - 40), 1.025);
    ...
    totalV3 = Math.pow((noteStat * 0.8) + (techStat * 0.8) + speedStat + (multiStat * 0.5) + (fillStat * 0.5), 1 + (diffmult / 100));
    console.log(data)
...
class SongData{
    ...
    CalculateDensity(noteMs:time_ms, arg1)
    {
        ...
        if (index < this.allBeatBuckets.length)
            this.allBeatBuckets[index] += arg1;
    }
    ...
    FinishDensityCalculation()
    {
        ...
        {
            if (this.buckets[i] > this.peakDensity)
                this.peakDensity = this.buckets[i];
    ...
    CheckForCollisions (noteTime:time_ms)
    {
        ...
        if (this.holdNoteTimings.length> 0)
        {
            ...
            for (var i = 0; i < this.holdNoteTimings.length; i++)
            {
                ...
                if (timing[1] >= noteTime)
                    newTimings.push(timing);
                
                if (noteTime >= timing[0] && noteTime < timing[1])
                    this.multiHoldNoteCount++;
            }
